...
def _direction_categories_report():
    ...
    # The unparseable-date check is the payment side's; both kinds agree on it.
    _, date_from, date_to = _apply_exam_date_filter(Examination.query, date_from, date_to)

    def _paid_lines(model, category_path):
        query = (
            model.query
            .join(model.examination)
    ...
    for part, entries, source, category_attr in parts:
        for entry in entries:
            ...
            # cash, or a legacy line with no method — defaults to cash
            method = 'terminal' if entry['is_terminal'] else 'cash'
            for target in (bucket[part], bucket):
                target[method] += entry['amount']
                target['discount_' + method] += entry['discount']
...
def _direction_categories_xlsx(rows, totals, date_from, date_to):
    ...
    money_cols = range(3, len(headers) + 1)

    def _money(m):
        return [float(m['cash']), float(m['terminal']),
                float(m['discount_cash']), float(m['discount_terminal'])]

    def _sums(m):
        cash = m['cash'] + m['discount_cash']
        terminal = m['terminal'] + m['discount_terminal']
        return [float(cash), float(terminal), float(cash + terminal)]
    ...
    # Two header rows: the group, then the column.
    group_row_idx = ws.max_row + 1
    col = 1
    group_row = []
    for title, width in groups:
        group_row += [title] + [''] * (width - 1)
    ws.append(group_row)
    for title, width in groups:
        if title and width > 1:
            ws.merge_cells(start_row=group_row_idx, start_column=col,
                           end_row=group_row_idx, end_column=col + width - 1)
        ws.cell(row=group_row_idx, column=col).alignment = center
        col += width
    header_row_idx = ws.max_row + 1
    ws.append(headers)
    for idx in (group_row_idx, header_row_idx):
...
def _analysis_payments_report():
    ...
    Returns (rows, totals, analysis_ids, date_from, date_to, analyses). `rows`
    is None until at least one analysis is picked; otherwise one bucket per
    selected analysis, ordered by analysis name. Each matching analysis line's
    amount goes to the cash or terminal bucket by payment method (missing
    method counts as cash), and its insurance 50% discount is accumulated
    separately."""
    analysis_ids = list(dict.fromkeys(request.args.getlist('analysis_ids', type=int)))
    date_from, date_to = _report_dates_with_today()

    analyses = (Analysis.query
                .filter(Analysis.is_active == True)
    ...
    if analysis_ids:
        query = (
            ExaminationAnalysis.query
            .join(ExaminationAnalysis.examination)
            .filter(
                Examination.is_paid == True,
    ...
    return rows, totals, analysis_ids, date_from, date_to, analyses
...
def reports_analysis_payments():
    ...
    if request.args.get('export') == 'xlsx' and rows is not None:
        return _analysis_payments_xlsx(rows, totals, date_from, date_to)

    return render_template(
        'main/reports/analysis_payments_report.html',
        rows=rows,
        totals=totals,
        analysis_ids=analysis_ids,
        date_from=date_from,
...
def _analysis_payments_xlsx(rows, totals, date_from, date_to):
    """Build an .xlsx workbook of the analysis payment report."""
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, PatternFill
    from openpyxl.utils import get_column_letter
    ...
    ws.append(['Analizler boýunça töleg hasabaty'])
    ws['A1'].font = Font(bold=True, size=14)
    if date_from or date_to:
        ws.append([f'Döwür: {date_from or "…"} — {date_to or "…"}'])
    ws.append([])
    ...
    fname = 'analysis_payments_report'
    if date_from or date_to:
        fname += f'_{date_from or "all"}_{date_to or "all"}'
    fname += '.xlsx'
